	...
	if (Process32First(hSnap, &procEntry))
	{
		do
		{
			for (size_t i = 0; i < illegalSoftware.size(); i++)
			{
				...
				{
					TerminateProcess(OpenProcess(PROCESS_TERMINATE, 0, procEntry.th32ProcessID), -1);
					return 1;
				}
			}
...
typedef char*(*__cdecl __fgets)(char*, int, FILE*);
...
int main()
{
	...
	srand(time(0));
	bool debuggerPresent = IsDebuggerPresent(); JUNK63 


	char input[256];
	char correctPasswordMsg[] = { 0x10, 0x3c, 0x21, 0x21, 0x36, 0x30, 0x27, 0x73, 0x23, 0x32, 0x20, 0x20, 0x24, 0x3c, 0x21, 0x37, 0xa, 0x00 }; JUNK73 
	...
	HMODULE ucrtbaseMod = GetModuleHandle(XorStr("ucrtbase.dll"));
	__fgets _fgets = (__fgets)GetProcAddress(ucrtbaseMod, XorStr("fgets"));
	...
	bool running = true;
	XORCipher(invalidPasswordMsg); JUNK18 
	while (running)
	{
		ticks = GetTickCount(); JUNK71 
		...
		_fgets(input, sizeof(input), stdin); JUNK76 
		...
		{
			exit(EXIT_BEING_DEBUGGED); JUNK51 
		}
